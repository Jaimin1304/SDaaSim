...
def dijkstra(
    nodes: Dict[str, Node], start_id: str, end_id: str
...
def init(skyway: Skyway):
    ...
    ########################################
    #  init your own data structures here  #
    ########################################
    ...
    for key, value in skyway.edges.items():
        edges[key] = Edge(
            nodes[value.leftNode.id], nodes[value.rightNode.id], value.totalLength
        )
